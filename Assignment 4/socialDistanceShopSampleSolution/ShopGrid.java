...
import java.util.concurrent.Semaphore; // ADDED

public class ShopGrid {
	private GridBlock [][] Blocks;
	private final int x;
	private final int y;
	private Semaphore lock; // ADDED
   private Semaphore lockEntrance; // ADDED
	...
	ShopGrid() throws InterruptedException {
		...
		this.lock = new Semaphore(SocialDistancingShop.max); // ADDED
      this.lockEntrance = new Semaphore(1); // ADDED
	}
	...
	ShopGrid(int x, int y, int [][] exitBlocks,int maxPeople) throws InterruptedException {
		...
		this.lock = new Semaphore(maxPeople); // ADDED
      this.lockEntrance = new Semaphore(1); // ADDED
	}
		...
		//try to move in 
   		...
   		GridBlock newBlock = Blocks[new_x][new_y];
   			...
   			if (newBlock.get())  {  //get successful because block not occupied 
   					if ((currentBlock.getX()==whereEntrance().getX()) && (currentBlock.getY()==whereEntrance().getY())){ // ADDED
                     lockEntrance.release();
                  }
                  if ((newBlock.getX()==whereEntrance().getX()) && (newBlock.getY()==whereEntrance().getY())){ // ADDED
                     lockEntrance.acquire();
                  }
